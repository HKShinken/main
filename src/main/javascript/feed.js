			...
			else
			{
					...
					con.query(sql,values, function (err, result, fields) {
												...
												var resultPage =  '<html><head><style> .center { color: rgba(140, 80, 80, 0.9); display: block; margin-left: auto; margin-right: auto; width: 50%; } .tinycircle { margin-left: 1; clip-path: circle(); } .spacing { margin: 8px 8px 0px 0px; } /*a { text-decoration: none; }*/ </style>'
																...
																+ '	<div>'
																+ '		<h1> Ciao ' + params.username +', di seguito il tuo feed </h1>'
												...
												for(i = 0; i < result.length ; i++)
												{
													...
													var timelinePage = "<a href=\"TIMELINE?username=" + params.username + "&pid=" + pid + "\">";
													var profilePage = "<a href=\"PROFILE?username=" + params.username + "&pid=" + pid + "&profile=";
													...
													if(result[i].cnt != 0)
													 ...
													 resultPage +='<div id="' + pid + '">'
																...
																+ '			   <div id="likesPost' + pid + '" hidden style="position:relative; left: 20;">	Piace a ' + listaProfili + ' </div>'
																+ '		</div>'
																+ '	</div>'
																+ '	</br>'
																+ ' <div id="mainDescr' + pid + '">DESCRIZIONE: ' + description + '</div>'
																+ ' </div>'
