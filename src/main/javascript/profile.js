...
function main(params)
{
    ...
    var values = [params.profile, post_id]
	
	var sqlSession = "select count(*) as cnt from user_session where username = ?";
	
	//La pagina contiene massimo 7 posts
			...
			else
			{
				con.query(sql,values, function (err, result, fields) {
											...
											for(i = 0; i < result.length && i < 7 ; i++)
											{
												...
												if(result[i].cnt != 0)
												 ...
												 resultPage +='<div id="' + loopPid + '">'
															...
															+ '		 <div> '
															+ '			   <a href="ADD_LIKE?username=' + params.username +'&pid=' + loopPid + '&r=1&profile=' + params.profile + '"><img src="' + imLikeButtonHtml + '" class = "spacing" alt="like"  width="20" height="20" /></a>'
											...
											if( result.length != 0 )
											{
												...
												var btnPrev = ( result[i].max_pid == post_id ? "" : '<input type=button value="<<" onClick="window.location.href=\'PROFILE?username=' + params.username + '&pid=' + post_id + '&profile=' + params.profile + '&b=1\'">');
					
												//post successivi hanno id inferiore -> specularmente non posso visualizzare pagine successive se non ci sono post di id inferiore rispetto all'ultimo considerato -> result[i]
												var btnNext = ( result[i].min_pid == result[i].post_id ? "" :'<input type=button value=">>" onClick="window.location.href=\'PROFILE?username=' + params.username + '&pid=' + result[i].post_id + '&profile=' + params.profile + '&b=0\'">');
